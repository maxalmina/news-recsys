...
def parse_news(source, target, bert_output_dir, category2int_path, word2int_path,
    ...
    def parse_row(row):
        new_row = [
            ...
            [0] * config.num_words_title, # title_entities
            [0] * config.num_words_abstract, # abstract_entities
            [0] * config.num_words_content,
        ]
        ...
        try:
            for i, w in enumerate(word_tokenize(row.content.lower())):
                if w in word2int:
                    new_row[7][i] = word2int[w]
                    #if w in local_entity_map:
                    #    new_row[6][i] = local_entity_map[w]
        except IndexError:
            pass
    ...
    if mode == 'train':
        ...
        for row in news.itertuples(index=False):
            ...
            for w in word_tokenize(row.content.lower()):
                if w not in word2freq:
                    word2freq[w] = 1
                else:
                    word2freq[w] += 1
...
if __name__ == '__main__':
    train_dir = './data/kcm/train'
    val_dir = './data/kcm/val'
    test_dir = './data/kcm/test'
